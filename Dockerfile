...
ARG BOOST_VERSION
ARG QT_VERSION
ARG BOOST_VERSION_=${BOOST_VERSION//./_}
WORKDIR /workspace
...
RUN  wget https://archives.boost.io/release/$BOOST_VERSION/source/boost_$BOOST_VERSION_.tar.gz && tar xf boost_$BOOST_VERSION_.tar.gz && mv boost_$BOOST_VERSION_ boost && cd boost && ./bootstrap.sh && ./b2 stage --stagedir=./ --with-headers
